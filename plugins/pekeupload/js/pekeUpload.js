...
(function($) {
  ...
  $.fn.pekeUpload = function(options){
    ...
    this.each(function() {
	  ...
	  if (options.theme == "registrasi"){
        var html = '<a href="javascript:void(0)"> <span class="icon-upload icon-white"></span> '+options.btnText2+'</a><div class="pekecontainer"></div>';
      }
      obj.after(html);
      obj.hide();
            ...
            success: function(data){
              ...
              obj.next('a').next('div').find('.pekeup-progress-bar:first').width(percent+'%');
                ...
                if (data.error==false){
				  ...
				  if((options.theme == "registrasi")&&(options.showErrorAlerts==true)){
                    obj.next('a').next('div').prepend('<div class="alert alert-error"><button type="button" class="close" data-dismiss="alert">&times;</button> '+data+'</div>');
                    bootstrapclosenotification();
                  }
                  error = false;
