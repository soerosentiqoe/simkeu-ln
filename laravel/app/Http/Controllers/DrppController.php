...
class DrppController extends AppModelController {
	var $table="d_drpp_kemlu";
	var $table_detail="d_drpp_detail_kemlu";
	var $view="v_drpp_kemlu";
	var $v_max_no="v_max_nodrpp";
    ...
    public function getTransaksi(Request $request) {
		...
		return response()->json(['data'=>DB::table('v_transaksi_drpp')->where($where)->get()]);
		
	}
    public function getMaxNo(Request $request){
		$where=['kdsatker'=>session('kdsatker'),'thang'=>session('thang')];
        $column='nodrpp';    
        return response()->json(['data'=>$this->getFirstRowByWhere($this->v_max_no,$where,$column) == 0 ? '1' : $this->getFirstRowByWhere($this->v_max_no,$where,$column)]);
	...
	{
		$aColumns = array('id','nodrpp','tgdrpp','kdgiat','nmjns');
		...
		if (isset($_GET['sSearch'])) {
			...
			if ((isset($sSearch)) && ($sSearch != '')) {
				$sWhere = " where nodrpp like '" . $sSearch . "%' or lower(kdgiat) like '%" . $sSearch . "%' ";
			}
		}
		...
		foreach( $rows as $row ) {
			...
			$output['aaData'][] = array(
				'<center>' . $row->nodrpp. '</center>',
                '<center>' . $row->tgdrpp . '</center>',
                '<center>' . $row->kdgiat. '</center>',
                '<center>' . $row->nmgiat . '</center>',
                $aksi
			);
		}
