...
class SpbyController extends AppModelController {
	var $table="d_spby_kemlu";
	var $view="v_spby_kemlu";
	var $v_max_no="v_max_nospby";
	...
	{
		$aColumns = array('id','nospby','tgspby','kdvalas','nilai','akun','kpd','uraian');
		...
		if (isset($_GET['sSearch'])) {
			...
			if ((isset($sSearch)) && ($sSearch != '')) {
				$sWhere = " where nospby like '" . $sSearch . "%' or lower(uraian) like '%" . $sSearch . "%' ";
			}
		}
		...
		foreach( $rows as $row ) {
			...
			$output['aaData'][] = array(
				'<center>' . $row->nospby. '</center>',
                '<center>' . $row->tgspby . '</center>',
                '<center>' . $row->kdvalas. '</center>',
                '<center>' . number_format($row->nilai) . '</center>',
                '<center>' . $row->kpd. '</center>',
